...
func main() {
	...
	deployedAddress := os.Getenv("MESSAGE_TRANSMITTER_ADDRESS")
	if deployedAddress == "" {
		log.Fatal("MESSAGE_TRANSMITTER_ADDRESS")
	}

	messageTransmitter, err := mt.NewMessageTransmitter(common.HexToAddress(deployedAddress), client)
	if err != nil {
		log.Fatal(err)
	}
	channel := make(chan *mt.MessageTransmitterMessageSent)
	watchOpts := &bind.WatchOpts{Context: context.Background(), Start: nil}
	sub, err := messageTransmitter.WatchMessageSent(watchOpts, channel)
	if err != nil {
		log.Fatal(err)
	}
